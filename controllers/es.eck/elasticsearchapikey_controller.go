...
func (r *ElasticsearchApikeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	isDeleted := !apikey.ObjectMeta.DeletionTimestamp.IsZero()

	if isDeleted && utils.SkipRemoteDelete(&apikey) {
		logger.Info("Skipping remote deletion, releasing finalizer.", "Resource", req.NamespacedName)
		return utils.ReleaseFinalizer(ctx, r.Client, &apikey, finalizer)
	}
